...
class Array(object):
    ...
    def swap(self, j, k): # Swap the values at 2
        if(0 <= j and j < self.__nItems and # Check if indices are
0 <= k and k < self.__nItems): # bounds, before
    ...
    def traverse(self, function=print): # Traverse all items
        for j in range(self.__nItems): # and apply a function
            function(self.__a[j])
    def __str__(self): # Special def for str()
        ans = "[" # Surround with square
        for i in range(self.__nItems): # Loop through items
            if len(ans) > 1: # Except next to left
                ans += ", " # separate items with comma
                ans += str(self.__a[i]) # Add string form of item
                ans += "]" # Close with right bracket
            return ans

    def bubbleSort(self): # Sort comparing adjacent
        for last in range(self.__nItems-1, 0, -1): # and bubble up
            for inner in range(last): # inner loop goes up to
                if self.__a[inner] > self.__a[inner+1]: # If elem less
                    self.swap(inner, inner+1) # than adjacent value,
    
    def selectionSort(self): # Sort by selecting min and
        for outer in range(self.__nItems-1): # swapping min to
            min = outer # Assume min is leftmost
        for inner in range(outer+1, self.__nItems): # Hunt to
...
# __a[min] is smallest among __a[outer]...__a[__nItems-1]
    ...
    def insertionSort(self): # Sort by repeated inserts
        ...
        self.__a[inner] = temp # Move marked elem to
'hole'